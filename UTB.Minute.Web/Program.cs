...
builder.Services.AddScoped<UTB.Minute.Web.Services.TokenProvider>();
builder.Services.AddScoped<IHttpClientFactory, UTB.Minute.Web.Services.ScopedHttpClientFactory>();
builder.Services.AddTransient<UTB.Minute.Web.Services.BearerTokenHandler>();

builder.Services.AddHttpClient("webapi", client =>
{
    client.BaseAddress = new Uri("https+http://webapi");
})
.AddHttpMessageHandler<UTB.Minute.Web.Services.BearerTokenHandler>();

builder.Services.AddSingleton<SseListenerService>();
builder.Services.AddHostedService(sp => sp.GetRequiredService<SseListenerService>());
...
.AddOpenIdConnect(OpenIdConnectDefaults.AuthenticationScheme, options =>
{
    ...
    options.ClientId = builder.Configuration["Keycloak:ClientId"] ?? "canteen-client";
    options.ClientSecret = builder.Configuration["Keycloak:ClientSecret"] ?? "canteen-secret";
            ...
            <div class='card shadow-lg p-4' style='max-width: 500px; width: 100%; border-radius: 12px;'>
                <div class='card-body text-center'>
                    ...
                    <p class='card-text text-muted small'>
                        ...
                        Odhlaste se a přihlaste se pod účtem s příslušnou rolí (např. cook pro zobrazení objednávek kuchařky).
                    </p>
                    <div class='mt-4'>
