...
public class BearerTokenHandler : DelegatingHandler
{
    ...
    protected override async Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
    {
        ...
        System.Console.WriteLine($"[DEBUG BearerTokenHandler] Outgoing request to {request.RequestUri} - Has Token: {(!string.IsNullOrEmpty(token))} (Length: {token?.Length ?? 0})");
...
public class ScopedHttpClientFactory : IHttpClientFactory
{
    ...
    public HttpClient CreateClient(string name)
    {
        ...
        if (name == "webapi")
        {
            ...
            var token = _tokenProvider.AccessToken;
            System.Console.WriteLine($"[DEBUG ScopedHttpClientFactory] Creating webapi client - Injecting Bearer Token: {!string.IsNullOrEmpty(token)} (Length: {token?.Length ?? 0})");
            if (!string.IsNullOrEmpty(token))
            {
